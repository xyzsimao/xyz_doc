...
- 87cdffa: **Drop `renderer` & `fields` API**
  ...
  'use client';
  import { defineClientConfig } from 'xyzdoc-openapi/ui/client';
...
- 40d0fa3: **Expect OpenAPI server to use `generateFiles()`**
  ...
  ```ts
  import { generateFiles } from "xyzdoc-openapi";
  import { openapi } from "@/lib/openapi";
...
- 189028a: Add `storageKeyPrefix` option to isolate `localStorage` for multiple API instances
  ...
  **Usage:**
  ...
  export default defineClientConfig({
    storageKeyPrefix: "xyzdoc-openapi-custom-",
  });
  ```
...
- ca09b6a: Core: Support accessing MDX plugins separately at `xyzdoc-core/mdx-plugins/*`
...
- 540027e: **Support `xyzdoc-openapi/css/preset.css` for Tailwind CSS**

  We highly recommend to use the following instead:

  ```css
  @import "tailwindcss";
  @import "xyzdoc-ui/css/neutral.css";
  @import "xyzdoc-ui/css/preset.css";
  /* do this */
  @import "xyzdoc-openapi/css/preset.css";
  ```
...
- ff12b53: **Move `APIPage` to `xyzdoc-openapi/ui`**
  ...
  ```tsx
  import defaultComponents from "xyzdoc-ui/mdx";
  import { APIPage } from "xyzdoc-openapi/ui";
  ...
  why: Next.js compiles the same module in different layers: route handlers, pages (which include browser bundle), and middleware (Edge Runtime). It avoids compiling React components of `xyzdoc-openapi` twice when you reference the OpenAPI server in a route handler.
...
- 3534a10: Move `xyzdoc-core` highlighting utils to `xyzdoc-core/highlight` and `xyzdoc-core/highlight/client`
...
- 1286a04: **Change interface for `useScalar`**
  ...
  ```tsx
  import { createOpenAPI } from "xyzdoc-openapi/server";
  import { APIPlayground } from "xyzdoc-openapi/scalar";
...
- ad143e1: Move UI implementation from `xyzdoc-ui` to `xyzdoc-openapi`.
  ...
  **migrate:**
  ...
  ```js
  import { createPreset, presets } from "xyzdoc-ui/tailwind-plugin";

  /** @type {import('tailwindcss').Config} */
  export default {
    content: [
      "./node_modules/xyzdoc-ui/dist/**/*.js",
      "./node_modules/xyzdoc-openapi/dist/**/*.js",
    ],
...
- 2b11c20: **Rename to Fumadocs**
  ...
  `next-docs-zeta` -> `xyzdoc-core`

  `next-docs-ui` -> `xyzdoc-ui`

  `next-docs-mdx` -> `xyzdoc-mdx`

  `@fuma-docs/openapi` -> `xyzdoc-openapi`

  `create-next-docs-app` -> `create-xyzdoc-app`
