...
- 9a3e2e8: **Require explicit cache**
  ...
  } from "xyzdoc-typescript";

  const generator = createGenerator({
    // add this!
    cache: createFileSystemGeneratorCache(".next/xyzdoc-typescript"),
  });
  ```
...
- f728e67: Support customising full type with `@xyzdocType` tag
...
- 5210f18: Support xyzdoc 16 in `peerDependencies`.
...
- 0d55667: Enforce `peerDeps` on xyzdoc deps
...
- b83d946: **Use `createGenerator` API**
  ...
  ```ts
  import { createGenerator } from "xyzdoc-typescript";
  ...
  ```tsx
  import { remarkAutoTypeTable, createTypeTable } from 'xyzdoc-typescript';
  ...
  ```tsx
  import { AutoTypeTable, remarkAutoTypeTable } from "xyzdoc-typescript";
...
- f75287d: **Introduce `xyzdoc-docgen` package.**

  Offer a better authoring experience for advanced use cases.
  - Move `remark-dynamic-content` and `remark-install` plugins to the new package `xyzdoc-docgen`.
  ...
  **Usage**
  ...
  ```ts
  import { remarkDocGen, fileGenerator } from "xyzdoc-docgen";
  ...
  **Migrate**
  ...
  ```ts
  import { remarkInstall } from "xyzdoc-docgen";
  ```
