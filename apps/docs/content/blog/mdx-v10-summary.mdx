...
```ts
import createMDX from 'xyzdoc-mdx/config';
...
Now you can generate types using `xyzdoc-mdx` command, it's recommended to set it as a `postinstall` script.
...
```ts
import { map } from '@/.map';
import { createMDXSource } from 'xyzdoc-mdx/runtime/next';
...
```ts
import { createMDXSource } from 'xyzdoc-mdx/runtime/next';
import type { InferMetaType, InferPageType } from 'xyzdoc-core/source';
import { loader } from 'xyzdoc-core/source';
...
```ts
import { defineDocs, defineCollections, frontmatterSchema } from 'xyzdoc-mdx/config';
import { z } from 'zod';
