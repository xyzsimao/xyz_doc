...
- **A better versioning policy:** Reduce abstraction for mitigating upstream changes, instead version the components directly (e.g. creating a new `xyzdoc-ui/layouts/v2/page` instead of a major UI change).
- **Add xyzdoc-native cloud features:** More official integrations like monitoring/analytics feature designed for Fumadocs.
