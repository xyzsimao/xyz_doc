...
```package-install
npm install @xyzdoc/cli --save-dev
```
...
import { createFromSource } from 'xyzdoc-core/search/server';
...
import { createMetadataImage } from 'xyzdoc-core/server';
import { source } from '@/lib/source';
...
```ts title="route.ts"
import { generateOGImage } from 'xyzdoc-ui/og';
...
In favour of this, the `getImageMeta` function from `xyzdoc-ui/og` has been removed.
...
```tsx title="page.tsx"
import { source } from '@/lib/source';
import { DocsCategory } from 'xyzdoc-ui/page';
...
```ts
import { DocsLayout } from 'xyzdoc-ui/layout';
import { HomeLayout } from 'xyzdoc-ui/home-layout';
import { HomeLayoutProps } from 'xyzdoc-ui/home-layout';
```
...
import { BaseLayoutProps } from 'xyzdoc-ui/layouts/shared';
```

### Twoslash UI

We moved Twoslash UI components to `xyzdoc-twoslash`.
...
```ts
import 'xyzdoc-twoslash/twoslash.css';

import { Popup } from 'xyzdoc-twoslash/ui';
```
...
The types from `xyzdoc-core/search/shared` is moved to `xyzdoc-core/server`.
