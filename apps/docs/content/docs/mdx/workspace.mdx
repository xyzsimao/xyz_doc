...
import { docs } from 'xyzdoc-mdx:collections/my-workspace/server';
```
...
import * as MyWorkspace from 'xyzdoc-mdx:collections/my-workspace/server';
...
- when a deployment is triggered on $A$:
  ...
  - each workspace generates its own collection entries, e.g. `xyzdoc-mdx:collections/{repo}/server`.
